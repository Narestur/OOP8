...
{   
    //ifstream Car_models;
    //Car_models.open("Cars.dat");
    
    for (int i = 0; i < 10; i++)
    {
        Car *type;
        string model[10] = {"ferrari","lamborgini","freedom","mersedez","royal","mitsubishi","katafalc","morio","JoJo","tavria"};
       ...
       char l[10];
        ...
        //int so=sizeof(l);
        //cout<< so<<endl;
        cars newCar{ getRandom(1990,2022),getRandom(1000,20000), getRandom(1,100),model[i], "Yellow", type};
        add_Car(newCar);
    }
    //cout << sAll;
    for (int i = 0; i < 5; i++)
    {
        string first[5] = { "Taras","Ivas","Lubava","Aliosha","Mayya"};
        string second[5] = { "Bulbaa","Muivan","Karashi","Gucul","Ribn" };
        string date[5] = { "12.11.2002","21.03.2001","28.12.2005","15.09.2004","9.05.2005" };
        ApplicationData newApp(getRandom(0, 100), first[i], second[i], date[i]);
        newApp.CarNumber = i;
        add_app(newApp);
    }
}
void SharingCar::add_Car(cars CarObj) 
    {
    //if (CarNum < 10) {
        CarList.push_back(CarObj);
       // CarNum++;
    //}
}

void SharingCar::add_Car(int year, int price, int CarRun, string model, string color)
{
    cars CarObj ={ year , price , CarRun , model , color };
    CarList.push_back(CarObj);
}


void SharingCar::add_app( ApplicationData appObj/*, bool newApp*/)
{
    ...
    //if (appNum < 10) {
        appList.push_back(appObj);
        //appNum++;
    //}
}

void SharingCar::write(cars carWrite[10], ApplicationData appWrite[10])
{
    ofstream CarInfo("Car.dat");
    CarInfo.write((char*)&carWrite, sizeof(cars[10]));
    ofstream appInfo("app.dat");
    appInfo.write((char*)&appWrite, sizeof(ApplicationData[10]));
}
void SharingCar::read()
{
    ...
    for (int i = 0; i < 4; i++) {
        app_info.read((char*)&appList[i], sizeof(ApplicationData));
    }
}
void SharingCar::service(cars car[10], ApplicationData app[10])
{
    int i = 0;
    cout << "Введіть \n1.Для запису \n2.Для читання\n";
    cin >> i;
...
{
    ApplicationData::get_number_country(number);
}
